<?php
    ...
    if (strlen($_SESSION['alogin']) == 0) {
        ...
        if (isset($_POST['submit'])) {
            $brand = trim($_POST['brand']); // Remove extra spaces
            $id    = intval($_GET['id']);   // Ensure ID is an integer

            // Validation rules
            if (empty($brand)) {
                $error = "Brand name cannot be empty.";
            } elseif (strlen($brand) < 2 || strlen($brand) > 50) {
                $error = "Brand name must be between 2 and 50 characters.";
            } elseif (! preg_match("/^[a-zA-Z ]+$/", $brand)) {
                $error = "Brand name can only contain letters and spaces.";
            } else {
                // Check if brand name already exists (excluding the current brand)
                $sqlCheck   = "SELECT id FROM tblbrands WHERE BrandName=:brand AND id != :id";
                $queryCheck = $dbh->prepare($sqlCheck);
                $queryCheck->bindParam(':brand', $brand, PDO::PARAM_STR);
                $queryCheck->bindParam(':id', $id, PDO::PARAM_INT);
                $queryCheck->execute();

                if ($queryCheck->rowCount() > 0) {
                    $error = "Brand name already exists. Please use a different name.";
                } else {
                    // Update brand
