<?php
    ...
    if (strlen($_SESSION['login']) == 0) {
        ...
        if (isset($_POST['submit'])) {
            $testimonial = trim($_POST['testimonial']); // Trim whitespace
            $email       = $_SESSION['login'];

            // Validation: Check if the testimonial is empty
            if (empty($testimonial)) {
                $error = "Testimonial cannot be empty.";
            } else {
                // Check if the same testimonial already exists
                $checkSql   = "SELECT COUNT(*) FROM tbltestimonial WHERE UserEmail = :email AND Testimonial = :testimonial";
                $checkQuery = $dbh->prepare($checkSql);
                $checkQuery->bindParam(':email', $email, PDO::PARAM_STR);
                $checkQuery->bindParam(':testimonial', $testimonial, PDO::PARAM_STR);
                $checkQuery->execute();
                $count = $checkQuery->fetchColumn();

                if ($count > 0) {
                    $error = "You have already submitted this testimonial.";
                } else {
